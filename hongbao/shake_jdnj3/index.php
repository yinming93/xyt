<?php
    ...
    if( !isset($_GET['code']) ){
		echo '操作失败！';
		header('Location:https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx461b5105da7629f1&redirect_uri=http://xytang88.com/test/shake_jdnj3/index.php&response_type=code&scope=snsapi_base&state=1#wechat_redirect');
		exit;
    }
	...
	//摇一摇部分
        ...
        function shake() {
            ...
            $("#shakeup").animate({ top: "50%" }, 700, function () {
                $("#shakeup").animate({ top: "40%" }, 700, function () {
...
*/
	...
	$.post("ajax.php",data,function(res){
		...
		if(obj.msg=="sorry"){
			...
			alert("恭喜您抢到 "+obj.sum+" 元红包！请至微信会话页面查收！")
		}
...
//----------------------------------------------------------------------------------
			...
			$("#shakedown").animate({ top: "50%" }, 700, function (){
				$("#shakedown").animate({ top: "56%" }, 700, function (){
				})
					//sound.play();
			})
		}
...
<tbody>
	...
	<tr id="controlbar">
		<td class="controlbar" onclick="javascript:shake();">
			...
			<img src="images/sdy.png" style="position:absolute;top: 78%;width: 30%;left: 35%;">
