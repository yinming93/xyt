...
/***/ function(module, exports, __webpack_require__) {
	...
	;(function () {
	  ...
	  var addEvent = element_test[ADD_EVENT_LISTENER] ? function (el, type, fn, useCapture) {
	  ...
	  var getAppVersion = function getAppVersion(ua) {
	    var version;
	    var regexMAPI = /MApi/;
	  ...
	  var getOSName = function getOSName(ua) {
	  ...
	  if (appVersion && Semver.gte(appVersion, '8.0.6')) {
	  ...
	  function _insureRequestId(callback) {
	    ...
	    var tinyDPApp = {
	      callback: function callback(callbackId, retValue) {
	      ...
	      _generateCallbackName: function _generateCallbackName(callbackId) {
	      ...
	      getRequestId: function getRequestId(opts) {
	      ...
	      getRequestIdNew: function getRequestIdNew(opts) {
	        var success = opts.success;
	        var callbackId = this._generateCallbackId();
	        win[this._generateCallbackName(callbackId)] = function (result) {
	          success && success(result);
	  ...
	  function xhr(src) {
	    var x = Xhr();
	    // evt / h5 domain will send to hls directly
	    if (!/^evt|h5/.test(location.host)) {
	  ...
	  mix(document_hippo, {
	    ...
	    pv: function pv(cityId, shopType) {
	  ...
	  var HIPPO_METHODS = {
	    _setPageId: function _setPageId(pageId) {
	      _pageId = pageId >>> 0;
	    },

	    _setCityId: function _setCityId(cityId) {
	      _cityId = cityId >>> 0;
	    },

	    _setShopType: function _setShopType(shopType) {
	      _shopType = shopType >>> 0;
	    },

	    _setPVInitData: function _setPVInitData(data) {
	      // run once
	    ...
	    _autoPV: function _autoPV(auto) {
	      auto_pv = auto;
	    },

	    _autoPageTiming: function _autoPageTiming(timing) {
	      auto_page_timing = timing;
	    },

	    // DP.data('hippo_perf_version', 'b1') ->
	    // _hip.push(['_setPageTimingVer', 'b1'])
	    _setPageTimingVer: function _setPageTimingVer(version) {
	      page_timing_version = version;
	    },

	    _setReferrer: function _setReferrer(referrer) {
	      ref = referrer;
	    },

	    _setHref: function _setHref(h) {
	      href = h;
	    },

	    _setRequestId: function _setRequestId(id) {
	      setCustomConst('reqid', id);
	    },

	    _setGuid: function _setGuid(id) {
	      setCustomConst('serverguid', id);
	    },

	    _setCustomConst: setCustomConst,

	    mv: function mv(data, ajax) {
	  ...
	  function bind_domready(fn) {
	    ...
	    if (doScroll) {
	      var poll_scroll = function poll_scroll() {
	        try {
	  ...
	  var removeEvent = element_test[REMOVE_EVENT_LISTENER] ? function (el, type, fn, useCapture) {
